...
import { SearchParams } from "../../src/types/searchTypes";
...
const queryParamsEquality = (test: string, queryParamsToConstruct: SearchParams) => {
  const testQueries = test.substring(1).split('&')
  const constructedQueries = getDRBQueryStringFromSearchParams(queryParamsToConstruct)
    .substring(1).split('&')
  return testQueries.every((queryParam: string) =>
...
describe('researchNowUtils', () => {
  describe('getDRBQueryStringFromSearchParams', () => {
    ...
    it('should handle field', () => {
      ...
      expect(queryParamsEquality(testQuery, { q: 'toast', field: 'title' })).toBe(true)
    })
    ...
    it('should handle keyword & subject query', () => {
      ...
      expect(queryParamsEquality(testQuery, { q: 'toast', filters: { subjectLiteral: 'Snacks' } })).toBe(true)
    })
    ...
    it('should handle contributor filter', () => {
      ...
      expect(queryParamsEquality(testQuery, { filters: { contributorLiteral: 'Poe' } })).toBe(true)
    })
    ...
    it('should handle lang filter', () => {
      ...
      expect(queryParamsEquality(testQuery, { filters: { language: 'lang:en' } })).toBe(true)
    })
    ...
    it('should handle dateAfter filter', () => {
      ...
      expect(queryParamsEquality(testQuery, { filters: { dateAfter: '2000' } })).toBe(true)
    })
    ...
    it('should handle dateBefore filter', () => {
      ...
      expect(queryParamsEquality(testQuery,{ filters: { dateBefore: '2020' } })).toBe(true)
    })
    ...
    it('should handle dateAfter & dateBefore filter', () => {
      ...
      expect(queryParamsEquality(testQuery, { filters: { dateAfter: '2000', dateBefore: '2020' } })).toBe(true)
    })
    ...
    it('should combine query params', () => {
      ...
      expect(queryParamsEquality(testQuery, { contributor: 'Poe', subject: 'corvids', q: 'Raven' }))
        .toBe(true)
    })
  })
})