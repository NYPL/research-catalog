import handler from "../../../../pages/api/account/settings/[id]"
import { updatePatronSettings } from "../../../../pages/api/account/helpers"
...
jest.mock("../../../../pages/api/account/helpers", () => {
  ...
  return {
    __esModule: true,
    ...originalModule,
    updatePatronSettings: jest.fn().mockResolvedValueOnce({
      status: "200",
      message: "Updated",
    }),
  }
...
describe("handler", () => {
  ...
  it("should return 403 if no authenticated patron", async () => {
    ...
    expect(updatePatronSettings).not.toHaveBeenCalled
  ...
  it("should call updatePatronSettings if authentication succeeds", async () => {
    req.query.id = "123456"
    ;(initializePatronTokenAuth as jest.Mock).mockResolvedValueOnce({
      decodedPatron: { sub: "123456" },
    })
    await handler(req as NextApiRequest, res as NextApiResponse)
    expect(updatePatronSettings).toHaveBeenCalled
  })
})
